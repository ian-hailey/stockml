...
def generate_buysell_signal(df_day_s, day, stop=10, resolution='M'):
    ...
    if df_pre_vol:
        ...
        buysell = pd.DataFrame(0.0, index=df_open.data.index, columns=['buysell'])
        nearest_zc = -1
        buysell_values = buysell['buysell'].values
...
#            print("row={} nearest_zc={} maDelta={}".format(rowIndex, nearest_zc, row.ma5grad))
            if nearest_zc >= rowIndex:
                buysell_values[rowIndex] = nearest_zc_ma5 - row[5]
        plot_buysell_signal(day, df_open.data['Close'].values, df_open.data['ma5'].values, buysell['buysell'].values, openIndex)
        buysell['buysell'].fillna(value=0.0, inplace=True)
        return buysell
    return None
    ...
    if thread_count > 1:
        with Pool(processes=thread_count) as pool:
            for i in range(days.size):
                ...
                pool.apply_async(generate_buysell_signal, args=(df_day_s, days[i], stop, 'S'), callback=buysell_signal_callback)
            pool.close()
            pool.join()
    else:
        for i in range(days.size):
    ...
    if thread_count > 1:
        with Pool(processes=thread_count) as pool:
            for i in range(days.size):
                dates_day_s = pd.date_range(days[i] + pd.DateOffset(hours=4), days[i] + pd.DateOffset(hours=stop), freq='S')
                df_day_s = df.daterange(dates_day_s)
                pool.apply_async(find_signals, args=(df_day_s, days[i], stop, 'M'), callback=find_signals_callback)
            pool.close()
            pool.join()
    else:
        for i in range(days.size):
