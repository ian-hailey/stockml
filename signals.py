...
class Signals(object):
    def __init__(self):
        self.signal = pd.DataFrame(columns=['zc'])

    def callback(self, results):
        self.signal = pd.concat([self.signal, results])

    def plot(self, day, close, ma5, signal, openIndex):
    ...
    def generate_zc(self, df_day_s, day, stop=10, resolution='M', require_pre_vol=False):
        ...
        if df_pre_vol or require_pre_vol is False:
            ...
            signal = pd.DataFrame(0.0, index=df_open.data.index, columns=['zc'])
            nearest_zc = -1
            signal_values = signal['zc'].values
    ...
    #        self.plot(day, df_open.data['Close'].values, df_open.data['ma5'].values, signal['zc'].values, openIndex)
            signal['zc'].fillna(value=0.0, inplace=True)
            return signal
        return None

    def zc_from_df(self, df, days, stop=10, thread_count=1, signal='zc'):
        if thread_count > 1:
            with Pool(processes=thread_count) as pool:
                for i in range(days.size):
                    ...
                    pool.apply_async(self.generate_zc, args=(df_day_s, days[i], stop, 'S'), callback=self.callback)
                pool.close()
                pool.join()
        else:
            for i in range(days.size):
